...
@section('header')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Lista de Sub Dimensiones</h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="">Menu Principal</a>
                        </li>
                        <li class="breadcrumb-item"><a href="{{ route('subdimensiones.index') }}">Sub Dimensiones</a>
                        </li>
                ...
                <div class="card">
                    <div class="card-header border-bottom p-1">
                        <div class="head-label">
                            <h6 class="mb-0">Lista de Sub Dimensiones</h6>
                        </div>
                        <div class="dt-action-buttons text-end">
                            <div class="dt-buttons d-inline-flex">
                                ...
                                <button class="btn btn-primary btn-toggle-sidebar w-100" data-bs-toggle="modal"
                                    ...
                                    <span class="align-middle">Registrar SubDimensión</span>
                                </button>
                            </div>
                        ...
                        <form id="search-form" class="mb-3">
                            <div class="input-group">
                                <input type="text" name="search" id="search-input" class="form-control"
                                    placeholder="Buscar por descripción de subdimensión">
                            ...
                            <tbody id="result-table">
                                ...
                                @foreach ($subdimensiones as $subdimensione)
                                    <tr>
                                        ...
                                        <td>
                                            {{ $subdimensione->descripcion ?? 'No existe descripción.....' }}
                                        </td>
                                        <td>
                                            {{ $subdimensione->dimensiones->descripcion ?? 'No existe descripción.....' }}
                                        </td>
                                        <td>
                                            <button class="btn {{ $subdimensione->estado == '1' ? 'btn-success' : 'btn-danger' }}">
                                                {{ $subdimensione->estado == '1' ? 'Habilitado' : 'Deshabilitado' }}
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('subdimensiones.destroy', $subdimensione) }}" method="POST"
                                                class="desactivar">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <a href="#" class="btn btn-primary dt-button create-new"
                                                    ...
                                                    data-id="{{ $subdimensione->id }}"  data-dimension="{{ $subdimensione->dimension_id }}"
                                                    data-descripcion="{{ $subdimensione->descripcion }}">
                                                ...
                                                {{-- @can('areas.destroy') --}}
                                                <button type="submit"
                                                    class="btn btn-danger {{ $subdimensione->estado == '2' ? 'disabled' : '' }}">
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('subdimensiones.activar', $subdimensione) }}" method="POST"
                                                    class="activar">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $subdimensione->estado == '1' ? 'disabled' : '' }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mt-1">
                                ...
                                <div id="result-info" class="dataTables_info">
                                    Mostrando {{ $subdimensiones->firstItem() }} a {{ $subdimensiones->lastItem() }} de
                                    {{ $subdimensiones->total() }} registros
                                </div>
                            ...
                            <div class="col-md-6">
                                <div id="result-pagination" class="dataTables_paginate">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item {{ $subdimensiones->previousPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $subdimensiones->previousPageUrl() }}">Anterior</a>
                                        </li>

                                        @if ($subdimensiones->currentPage() > 3)
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $subdimensiones->url(1) }}">1</a>
                                            </li>
                                            @if ($subdimensiones->currentPage() > 4)
                                        ...
                                        @foreach ($subdimensiones->getUrlRange(max($subdimensiones->currentPage() - 2, 1), min($subdimensiones->currentPage() + 2, $subdimensiones->lastPage())) as $page => $url)
                                            <li
                                                class="page-item {{ $page == $subdimensiones->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        @if ($subdimensiones->currentPage() < $subdimensiones->lastPage() - 2)
                                            @if ($subdimensiones->currentPage() < $subdimensiones->lastPage() - 3)
                                                <li class="page-item disabled">
                                                    <span class="page-link">...</span>
                                                </li>
                                            @endif
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $subdimensiones->url($subdimensiones->lastPage()) }}">{{ $subdimensiones->lastPage() }}</a>
                                            </li>
                                        @endif

                                        <li class="page-item {{ $subdimensiones->nextPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $subdimensiones->nextPageUrl() }}">Siguiente</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body pb-5 px-sm-5 pt-50">
                        <div class="text-center mb-2">
                            <h1 class="mb-1">Registrar Sub Dimensión</h1>
                            <p></p>
                        </div>


                        <form id="editAddForm" class="row gy-1 pt-75 formguardar" action="{{ route('subdimensiones.store') }}"
                            method="POST">
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 mt-1">
                                        ...
                                        <select name="dimension_id" id="dimension_id" class="form-select" required
                                            style="width: 100%">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dt-button.create-new').click(function() {
                ...
                $('#edit_dimension_id').val(dimension)

                $('#editEvent').modal('show');
                var actionUrl = "{{ route('subdimensiones.update', ':id') }}";
