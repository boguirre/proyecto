...
@section('header')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Lista de Rango Edad</h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="">Menu Principal</a>
                        </li>
                        <li class="breadcrumb-item"><a href="{{ route('rango-edad.index') }}">Rango Edad</a>
                        </li>
                ...
                <div class="card">
                    <div class="card-header border-bottom p-1">
                        <div class="head-label">
                            <h6 class="mb-0">Lista de Rango Edad</h6>
                        </div>
                        <div class="dt-action-buttons text-end">
                            <div class="dt-buttons d-inline-flex">
                                {{-- <a href="{{ route('rango-edade') }}" class="dt-button create-new btn btn-primary">
                                ...
                                <button class="btn btn-primary btn-toggle-sidebar w-100" data-bs-toggle="modal"
                                    ...
                                    <span class="align-middle">Registrar Rango Edad</span>
                                </button>
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <form id="search-form" class="mb-3">
                            <div class="input-group">
                                <input type="text" name="search" id="search-input" class="form-control"
                                    placeholder="Buscar por descripción de edad">
                        ...
                        <table id="result-table" class="table table-responsive  datatables-basic dtr-column collapsed ">
                            ...
                            <tbody id="result-table">
                                ...
                                @foreach ($rangoEdades as $rango_edad)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            {{ $rango_edad->descripcion ?? 'No existe descripción.....' }}
                                        </td>
                                        <td>
                                            <button class="btn {{ $rango_edad->estado == '1' ? 'btn-success' : 'btn-danger' }}">
                                                {{ $rango_edad->estado == '1' ? 'Habilitado' : 'Deshabilitado' }}
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('rango-edad.destroy', $rango_edad) }}" method="POST"
                                                class="desactivar">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <a href="#" class="btn btn-primary dt-button create-new"
                                                    ...
                                                    data-id="{{ $rango_edad->id }}" {{-- data-nombre="{{ $dimension->nombre }}" --}}
                                                    data-descripcion="{{ $rango_edad->descripcion }}">
                                                ...
                                                {{-- @can('areas.destroy') --}}
                                                <button type="submit"
                                                    class="btn btn-danger {{ $rango_edad->estado == '2' ? 'disabled' : '' }}">
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('rango-edad.activar', $rango_edad) }}" method="POST"
                                                    class="activar">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $rango_edad->estado == '1' ? 'disabled' : '' }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mt-1">
                                ...
                                <div id="result-info" class="dataTables_info">
                                    Mostrando {{ $rangoEdades->firstItem() }} a {{ $rangoEdades->lastItem() }} de
                                    {{ $rangoEdades->total() }} registros
                                </div>
                            ...
                            <div class="col-md-6">
                                <div id="result-pagination" class="dataTables_paginate">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item {{ $rangoEdades->previousPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $rangoEdades->previousPageUrl() }}">Anterior</a>
                                        </li>

                                        @if ($rangoEdades->currentPage() > 3)
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $rangoEdades->url(1) }}">1</a>
                                            </li>
                                            @if ($rangoEdades->currentPage() > 4)
                                        ...
                                        @foreach ($rangoEdades->getUrlRange(max($rangoEdades->currentPage() - 2, 1), min($rangoEdades->currentPage() + 2, $rangoEdades->lastPage())) as $page => $url)
                                            <li
                                                class="page-item {{ $page == $rangoEdades->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        @if ($rangoEdades->currentPage() < $rangoEdades->lastPage() - 2)
                                            @if ($rangoEdades->currentPage() < $rangoEdades->lastPage() - 3)
                                                <li class="page-item disabled">
                                                    <span class="page-link">...</span>
                                                </li>
                                            @endif
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $rangoEdades->url($rangoEdades->lastPage()) }}">{{ $rangoEdades->lastPage() }}</a>
                                            </li>
                                        @endif

                                        <li class="page-item {{ $rangoEdades->nextPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $rangoEdades->nextPageUrl() }}">Siguiente</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body pb-5 px-sm-5 pt-50">
                        <div class="text-center mb-2">
                            <h1 class="mb-1">Registrar Rango Edad</h1>
                            <p></p>
                        </div>


                        <form id="editAddForm" class="row gy-1 pt-75 formguardar" action="{{ route('rango-edad.store') }}"
                            method="POST">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dt-button.create-new').click(function() {
                ...
                var actionUrl = "{{ route('rango-edad.update', ':id') }}";
