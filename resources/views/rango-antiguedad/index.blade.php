...
@section('header')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Lista de Rango Antiguedad</h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="">Menu Principal</a>
                        </li>
                        <li class="breadcrumb-item"><a href="{{ route('rango-antiguedad.index') }}">Rango Antiguedad</a>
                        </li>
...
@section('content')
    <section id="basic-datatable">
                ...
                <div class="card">
                    <div class="card-header border-bottom p-1">
                        <div class="head-label">
                            <h6 class="mb-0">Lista de Rango Antiguedad</h6>
                        </div>
                        <div class="dt-action-buttons text-end">
                            <div class="dt-buttons d-inline-flex">
                                {{-- <a href="{{ route('rango-antiguedade') }}" class="dt-button create-new btn btn-primary">
                                ...
                                <button class="btn btn-primary btn-toggle-sidebar w-100" data-bs-toggle="modal"
                                    ...
                                    <span class="align-middle">Registrar Rango Antiguedad</span>
                                </button>
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <table id="result-table" class="table table-responsive  datatables-basic dtr-column collapsed ">
                            ...
                            <tbody id="result-table">
                                ...
                                @foreach ($rangoAntiguedades as $rango_antiguedad)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            {{ $rango_antiguedad->descripcion ?? 'No existe descripción.....' }}
                                        </td>
                                        <td>
                                            <button class="btn {{ $rango_antiguedad->estado == '1' ? 'btn-success' : 'btn-danger' }}">
                                                {{ $rango_antiguedad->estado == '1' ? 'Habilitado' : 'Deshabilitado' }}
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('rango-antiguedad.destroy', $rango_antiguedad) }}" method="POST"
                                                class="desactivar">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <a href="#" class="btn btn-primary dt-button create-new"
                                                    ...
                                                    data-id="{{ $rango_antiguedad->id }}" {{-- data-nombre="{{ $dimension->nombre }}" --}}
                                                    data-descripcion="{{ $rango_antiguedad->descripcion }}">
                                                ...
                                                {{-- @can('areas.destroy') --}}
                                                <button type="submit"
                                                    class="btn btn-danger {{ $rango_antiguedad->estado == '2' ? 'disabled' : '' }}">
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('rango-antiguedad.activar', $rango_antiguedad) }}" method="POST"
                                                    class="activar">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $rango_antiguedad->estado == '1' ? 'disabled' : '' }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mt-1">
                                ...
                                <div id="result-info" class="dataTables_info">
                                    Mostrando {{ $rangoAntiguedades->firstItem() }} a {{ $rangoAntiguedades->lastItem() }} de
                                    {{ $rangoAntiguedades->total() }} registros
                                </div>
                            ...
                            <div class="col-md-6">
                                <div id="result-pagination" class="dataTables_paginate">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item {{ $rangoAntiguedades->previousPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $rangoAntiguedades->previousPageUrl() }}">Anterior</a>
                                        </li>

                                        @if ($rangoAntiguedades->currentPage() > 3)
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $rangoAntiguedades->url(1) }}">1</a>
                                            </li>
                                            @if ($rangoAntiguedades->currentPage() > 4)
                                        ...
                                        @foreach ($rangoAntiguedades->getUrlRange(max($rangoAntiguedades->currentPage() - 2, 1), min($rangoAntiguedades->currentPage() + 2, $rangoAntiguedades->lastPage())) as $page => $url)
                                            <li
                                                class="page-item {{ $page == $rangoAntiguedades->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        @if ($rangoAntiguedades->currentPage() < $rangoAntiguedades->lastPage() - 2)
                                            @if ($rangoAntiguedades->currentPage() < $rangoAntiguedades->lastPage() - 3)
                                                <li class="page-item disabled">
                                                    <span class="page-link">...</span>
                                                </li>
                                            @endif
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $rangoAntiguedades->url($rangoAntiguedades->lastPage()) }}">{{ $rangoAntiguedades->lastPage() }}</a>
                                            </li>
                                        @endif

                                        <li class="page-item {{ $rangoAntiguedades->nextPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link"
                                                href="{{ $rangoAntiguedades->nextPageUrl() }}">Siguiente</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body pb-5 px-sm-5 pt-50">
                        <div class="text-center mb-2">
                            <h1 class="mb-1">Registrar Rango Antiguedad</h1>
                            <p></p>
                        </div>


                        <form id="editAddForm" class="row gy-1 pt-75 formguardar" action="{{ route('rango-antiguedad.store') }}"
                            method="POST">
        ...
        @include('rango-antiguedad.modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dt-button.create-new').click(function() {
                ...
                var actionUrl = "{{ route('rango-antiguedad.update', ':id') }}";
